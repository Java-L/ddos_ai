...
class AttackSimulator:
    ...
    def dos_attack(self, duration=30):
        """模拟DoS攻击"""
        print(f"[DoS攻击] 开始对 {self.target_ip}:{self.target_port} 进行DoS攻击，持续{duration}秒")
        self.running = True
    ...
    def port_scan(self, start_port=1, end_port=1000):
        """模拟端口扫描攻击"""
        print(f"[端口扫描] 开始扫描 {self.target_ip} 的端口 {start_port}-{end_port}")
    ...
    def web_attack(self, duration=20):
        ...
        while (time.time() - start_time) < duration:
            try:
                ...
                print(f"Web攻击请求: {url} - 状态码: {response.status_code}")
    ...
    def bot_simulation(self, duration=25):
        """模拟僵尸网络行为"""
        print(f"[僵尸网络] 开始模拟僵尸网络行为，持续{duration}秒")
    ...
    def infiltration_attack(self, duration=15):
        """模拟渗透攻击"""
        print(f"[渗透攻击] 开始模拟渗透攻击，持续{duration}秒")
        ...
        while (time.time() - start_time) < duration:
            try:
                ...
                for username in usernames[:2]:  # 限制数量
                    for password in passwords[:2]:
                        try:
                            ...
                            print(f"暴力破解尝试: {username}:{password} - 状态: {response.status_code}")
                        except:
                            pass
...
def generate_normal_traffic(target_ip="127.0.0.1", target_port=8000, duration=60):
    ...
    while (time.time() - start_time) < duration:
        try:
            ...
            response = requests.get(url, headers=headers, timeout=5)
            print(f"正常请求: {url} - 状态码: {response.status_code}")
...
def main():
    parser = argparse.ArgumentParser(description='网络攻击模拟器')
    ...
    parser.add_argument('--normal', action='store_true', help='同时生成正常流量')
